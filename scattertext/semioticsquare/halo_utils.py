...
def term_coordinates_to_halo(term_coordinates_df: pd.DataFrame, num_terms: int = 5) -> dict:
    rad_coords = add_radial_parts_and_mag_to_term_coordinates(
        term_coordinates_df=term_coordinates_df
    )
    selection_to_terms = dict(
        rad_coords.sort_values(by='Mag', ascending=False).groupby('Part').apply(
            lambda gdf: list(gdf.iloc[:num_terms].index)
        )
    )
    for selection in FINE_PART_TO_SELECTION.values():
        if selection not in selection_to_terms:
            selection_to_terms[selection] = []
    return selection_to_terms
...
def add_radial_parts_and_mag_to_term_coordinates(term_coordinates_df: pd.DataFrame) -> pd.DataFrame:
    ...
    radial_term_coordinates_df = term_coordinates_df[['x', 'y']].assign(
        x=lambda df: stretch_neg1_to_1(df.x),
        y=lambda df: stretch_neg1_to_1(df.y)
    ).assign(
